import { SERVICES_PAGE_LIST } from '../data/products';
    ...
    <PageTransition>
      <div className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          {SERVICES_PAGE_LIST.map((service) => {
            if (service.variant === 'featured') {
              return (
                <div key={service.id} className="md:col-span-8 group relative overflow-hidden bg-surface-container-low rounded-3xl p-8 md:p-12 transition-all duration-500 hover:bg-surface-container-high">
                  <div className="flex flex-col h-full justify-between relative z-10">
                    <div>
                      ...
                      <h3 className="text-3xl font-bold font-headline mb-4">{service.name}</h3>
                      <p className="text-on-surface-variant text-lg max-w-md mb-8">{service.description}</p>
            ...
            if (service.variant === 'secondary') {
              return (
                <div key={service.id} className="md:col-span-4 bg-secondary-container rounded-3xl p-8 transition-all duration-500 hover:shadow-2xl hover:shadow-secondary/10">
                  <div className="bg-secondary/10 w-14 h-14 rounded-xl flex items-center justify-center mb-6">
                    <span className="material-symbols-outlined text-secondary text-2xl">{service.icon}</span>
                  </div>
                  <h3 className="text-2xl font-bold font-headline mb-3 text-on-secondary-container">{service.name}</h3>
                  <p className="text-on-secondary-container/80 text-sm mb-6 leading-relaxed">{service.description}</p>
                  <a className="text-secondary font-bold text-sm inline-flex items-center gap-1 hover:underline underline-offset-4" href={service.link}>
            ...
            if (service.variant === 'video') {
              return (
                <div key={service.id} className="md:col-span-4 bg-surface-container-lowest rounded-3xl p-8 transition-all duration-500 hover:bg-surface-container-low group">
                  <div className="aspect-video w-full mb-6 rounded-2xl overflow-hidden bg-surface-container">
                    <img className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-700" src={service.image} alt={service.name} />
                  </div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="material-symbols-outlined text-error">{service.icon}</span>
                  ...
                  <a className="text-primary font-bold text-sm" href={service.link}>En savoir plus</a>
                </div>
              );
            }

            if (service.variant === 'chat') {
              return (
                <div key={service.id} className="md:col-span-4 bg-surface-container-low rounded-3xl p-8 transition-all duration-500 hover:translate-y-[-4px]">
                  <div className="bg-tertiary-container w-12 h-12 rounded-full flex items-center justify-center mb-6">
                    <span className="material-symbols-outlined text-on-tertiary-container text-xl">{service.icon}</span>
                  </div>
                  <h3 className="text-xl font-bold font-headline mb-3">{service.name}</h3>
                  <p className="text-on-surface-variant text-sm mb-6">{service.description}</p>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-tertiary"></div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-tertiary">{service.tags?.[0]}</span>
                  </div>
                </div>
              );
            }

            if (service.variant === 'note') {
              return (
                <div key={service.id} className="md:col-span-4 group bg-surface-container-lowest rounded-3xl p-8 transition-all duration-500 hover:bg-white overflow-hidden relative">
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                      <span className="material-symbols-outlined text-on-surface-variant">{service.icon}</span>
                      <h3 className="text-xl font-bold font-headline">{service.id === 'notes' ? 'Etherpad' : service.name}</h3>
                    </div>
                    <p className="text-on-surface-variant text-sm mb-6">{service.description}</p>
            ...
            if (service.variant === 'horizontal') {
              return (
                <div key={service.id} className="md:col-span-6 bg-surface-container-lowest rounded-3xl p-8 flex flex-col md:flex-row items-center gap-8">
                  <div className="w-full md:w-1/3">
                    <img className="rounded-2xl shadow-lg" src={service.image} alt={service.name} />
                  </div>
